# This file was *autogenerated* from the file root_system.sage
...
class RootSystem(SageObject):
    ...
    def __init__(self, n, A, D):
        ...
        self._ambient_space = VectorSpace(QQ,_sage_const_2 *self._rank)
        B = self._ambient_space.basis()

        self._fundamental_weights = []
        for i in range(_sage_const_0 ,self._rank):
        ...
        self._simple_roots = []
        for i in range(self._rank,_sage_const_2 *self._rank):
        ...
        for i in range(_sage_const_0 ,self._rank):
            M=[]
            for r in range(_sage_const_0 ,_sage_const_2 *self._rank):
                if r == self._rank+i:
                    M.append(list(identity_matrix(QQ,self._rank).row(i))+list(self._cartan_matrix.row(i)))
                else:
                    M.append(list(self._zero))
            self._simple_reflections.append(identity_matrix(QQ,_sage_const_2 *self._rank)-matrix(QQ,M))
    ...
    def is_finite(self):
        ...
        if type(ct[_sage_const_0 ]) == str and len(ct) == _sage_const_2 :
            return true
        return false 
    ...
    def Weyl_character_formula(self, highest_weight):
        ...
        numer = self._ambient_char_ring(_sage_const_0 )
        denom = self._ambient_char_ring(_sage_const_0 )
    ...
    def weight_multiplicity(self, highest_weight, weight):
        ...
        denom_exp = sum([denom._mpoly_dict_recursive().keys()[_sage_const_0 ][i]*self.fundamental_weight(i) for i in xrange(self._rank)])
    ...
    def braid_action(self, i, seq):
        ...
        temp = copy(seq)
        if i > _sage_const_0 :
            new_root = self.reflection_across(seq[i])*seq[i-_sage_const_1 ]
        if i < _sage_const_0 :
            new_root = self.reflection_across(seq[-i-_sage_const_1 ])*seq[-i]
        neg_switch = false
        for j in range(_sage_const_0 ,_sage_const_2 *self._rank):
            if new_root[j] < _sage_const_0 :
                neg_switch = true
        if neg_switch:
            new_root *= -_sage_const_1 
        if i > _sage_const_0 :
            temp[i-_sage_const_1 ] = temp[i]
            temp[i] = new_root
        if i < _sage_const_0 :
            temp[-i] = temp[-i-_sage_const_1 ]
            temp[-i-_sage_const_1 ] = new_root
        #print temp
        return temp
    ...
    def commutation_matrix_from_list(self, bfi):
        ...
        """
        C = []
        for i in range(_sage_const_0 ,self.__ngens):
            row = []
            for j in range(_sage_const_0 ,self.__ngens):
                sum = _sage_const_0 
                for k in range(_sage_const_0 ,self.__ngens):
                    sum += self._symmetrizing_matrix[i][k]*self._cartan_matrix[k][j]
                row.append(sum)
            C.append(row)
        M = []
        len = bfi.__len__()
        for i in range(_sage_const_0 ,len):
            row = []
            for j in range(_sage_const_0 ,len):
                if i < j:
                    row.append(C[bfi[i]][bfi[j]])
                if i == j:
                    row.append(_sage_const_0 )
                if j < i:
                    row.append(-C[bfi[i]][bfi[j]])
            M.append(row)
        return matrix(QQ,M)
    ...
    def decompose_root(self, root):
        ...
        if init_height <= _sage_const_0 :
            sequences   .append([])
            return sequences
        large_components = []
        max_component = _sage_const_0 
        for i in range(self._rank,_sage_const_2 *self._rank):
    ...
    def is_decreasing_cuspidal(self, list, order):
        ...
        for i in range(_sage_const_0 ,cuspidal_list.__len__()-_sage_const_1 ):
            elt1 = cuspidal_list[i]
            elt2 = cuspidal_list[i+_sage_const_1 ]
            #print (elt1,elt2)
            min_len = elt1.__len__()
            if elt2.__len__() < min_len:
                min_len = elt2.__len__()
            equality_switch = true
            for j in range(_sage_const_0 ,min_len):
    ...
    def is_dominant(self, weight):
        ...
        for i in range(_sage_const_0 ,self._rank):
            if equiv_weight[i] < _sage_const_0 :
                return False
        return True   
    ...
    def latex_coroot(self, coroot):
        ...
        output=""
        for i in range(self._rank,_sage_const_2 *self._rank):
            if coroot[i]!=_sage_const_0 :
                if output != "":
                    if coroot[i] > _sage_const_0 :
                        output += "+"
                if coroot[i]!=_sage_const_1 :
                    if coroot[i] != -_sage_const_1 :
                        output += str(coroot[i])
                    else:
                        output += "-"
                output += "\\alpha^\\vee_{" + str(i+_sage_const_1 ) + "}"
        return output
        ...
        for i in range(self._rank,_sage_const_2 *self._rank):
            if root[i]!=_sage_const_0 :
                if output != "":
                    if root[i] > _sage_const_0 :
                        output += "+"
                if root[i]!=_sage_const_1 :
                    if root[i] != -_sage_const_1 :
                        output += str(root[i])
                    else:
                        output += "-"
                output += "\\alpha_{" + str(i+_sage_const_1 ) + "}"
        return output
    ...
    def latex_weight(self, weight):
        ...
        """
        output=""
        for i in range(_sage_const_0 ,self.rank):
            if weight[i]!=_sage_const_0 :
                if output != "":
                    if weight[i] > _sage_const_0 :
                        output += "+"
                if weight[i]!=_sage_const_1 :
                    if weight[i] != -_sage_const_1 :
                        output += str(weight[i])
                    else:
                        output += "-"
                output += "\\omega_{" + str(i+_sage_const_1 ) + "}"
        for i in range(self._rank,_sage_const_2 *self._rank):
            if weight[i]!=_sage_const_0 :
                if output != "":
                    if weight[i] > _sage_const_0 :
    ...
    def pairing(self, root, weight):
        return _sage_const_2 *self.symmetric_pairing(root,weight)/self.symmetric_pairing(root,root)
    ...
    def phi_strings(self, bfi):
        ...
        bfa = []
        m = bfi.__len__()
        for ell in range(_sage_const_0 ,m):
            weight = self.w(ell+_sage_const_1 ,bfi)*self._fundamental_weights[bfi[ell]]
            string = []
            for k in range(_sage_const_0 ,ell+_sage_const_1 ):
                string.append(-self.symmetric_pairing(self.w(k+_sage_const_1 ,bfi)*self._simple_coroots[bfi[k]],weight))
            for k in range(ell+_sage_const_1 ,m):
                string.append(_sage_const_0 )
            bfa.append(string)
        return bfa 
    ...
    def reflection_across(self, root):
        ...
        M = Matrix(QQ,M)
        return identity_matrix(QQ,_sage_const_2 *self._rank)-M.transpose()
    ...
    def string(self, bfi, weight):
        ...
        m = bfi.__len__()
        for ell in range(_sage_const_0 ,m):
            bfa.append(self.symmetric_pairing(self.w(ell+_sage_const_1 ,bfi)*self._simple_coroots[bfi[ell]],weight))
        return bfa
    ...
    def symmetric_pairing(self, root, weight):
        ...
        for i in range(_sage_const_0 ,self._rank):
            ...
            for j in range(_sage_const_0 ,self._rank):
    ...
    def w(self, ell, bfi):
        ...
        """
        element = identity_matrix(QQ,_sage_const_2 *self._rank)
        for i in range(_sage_const_0 ,ell):
            element = self._simple_reflections[bfi[ell-i-_sage_const_1 ]]*element
        return element
